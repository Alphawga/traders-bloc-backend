...
const CompanyDetailsForm = () => {
  ...
  const companyForm = useForm<z.infer<typeof companyDetailsSchema>>({
    ...
    defaultValues: {
      ...
      business_ownership_percentage:
        user?.business_ownership_percentage || undefined,
  ...
  const onSubmitCompanyDetails = async (
    data: z.infer<typeof companyDetailsSchema>
  ) => {
    try {
    ...
    <div className="container mx-auto py-10">
      ...
      <Card>
        ...
        <CardContent>
          <Form {...companyForm}>
            <form
              onSubmit={companyForm.handleSubmit(onSubmitCompanyDetails)}
              className="space-y-6"
            >
              <FormField
...
export default function KYB() {
  ...
  useEffect(() => {
    if (user?.kyc_documents) {
      requiredDocuments.forEach((doc) => {
        const userDoc = user.kyc_documents.find(
          (userDoc) => userDoc.document_type === doc.id
        );
        if (userDoc) {
  ...
  const onSubmit = async (data: FormValues) => {
    // const requiredDocsUploaded = requiredDocumentChecker
    //   .filter(
    //     (doc) =>
    //       doc.id !== "registrationNumber" &&
    //       doc.id !== "taxInformation" &&
    //       doc.id !== "incorporationDocuments"
    //   ) // Exclude optional docs
    //   .every((doc) => Boolean(data[doc.id as keyof FormValues])); // Check if all required docs are uploaded
